...
followersService.follow = (user, followed_user) => {
    db.none('INSERT INTO followers (person_whos_following_id,person_being_followed_id) VALUES (${user}, ${followed_user})', {user,followed_user})
}

followersService.unfollow = (user, unfollowed_user) => {
    db.none('DELETE FROM followers WHERE person_whos_following_id=${user} AND person_being_followed_id=${unfollowed_user}', {user, unfollowed_user})
}
